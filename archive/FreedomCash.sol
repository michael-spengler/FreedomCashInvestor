...
Wish:           May everyone who reads this with the best of intentions always have enough 
...
contract FreedomCash is ERC20 {
    ...
    struct Attestation { // once EAS is ready on mainnet we can integrate via Freedom Enterprise
        address voter;
    ...
    function voteForInvestmentIn(address scAddress, uint256 fCBuyPrice, uint256 fCAmount) public payable {
        ...
        if (investmentBudget >= (99 * 10**15)) {
            ...
            investmentCandidates[iCIDs[scAddress]].score = 0;           
        } 
        aCounter = aCounter + 1;
    ...
    function voteForPublicGood(address publicGoodWallet, uint256 fCBuyPrice, uint256 fCAmount) public payable {
        ...
        if(publicGoodsFundingBudget >= (99 * 10**15)) { 
            ...
            publicGoodCandidates[pGCIDs[winner]].score = 0;      
    ...
    function getAmountOutMinimum(address tIn, address tOut, uint256 aIn, uint24 poolFee, uint24 maxSlip) public view returns(uint256) {
        ...
        uint256 expectedOutputAmount = Math.mulDiv(aIn, 10**ERC20(tIn).decimals(),  price);
        uint256 amountOutMinimum = expectedOutputAmount - Math.mulDiv(expectedOutputAmount, maxSlip, 1000);
    ...
    function getInvestmentPriceForAsset(address t1, address poolAddress) public view returns(uint256) {
        IUniswapV3Pool pool = IUniswapV3Pool(poolAddress);
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();
        if (sqrtPriceX96 == 0) { revert UnreasonableRequest(); }
        uint256 amount0 = Math.mulDiv(pool.liquidity(), FixedPoint96.Q96, sqrtPriceX96);
    ...
    function swipSwapV3Service(address tIn, address tOut,uint24 poolFee, uint256 amountOutMinimum) public payable returns(uint256 aOut) {
        ...
        aOut = swapRouter.exactInputSingle{value: msg.value}(params);
        reconcileAndClear();        
    } 
